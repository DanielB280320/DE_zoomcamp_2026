...
normalized_trips AS ( -- Normalize column names from raw data (cast, coalesce, rename)
  SELECT
    ...
    CAST(COALESCE(tpep_pickup_datetime) AS TIMESTAMP) AS pickup_time,
    CAST(COALESCE(tpep_dropoff_datetime) AS TIMESTAMP) AS dropoff_time,
    passenger_count,
    trip_distance,
    store_and_fwd_flag,
  ...
  WHERE 1=1
    ...
    AND DATE_TRUNC(CAST(COALESCE(tpep_pickup_datetime) AS TIMESTAMP), MONTH) BETWEEN DATE_TRUNC(CAST('{{ start_datetime }}' AS TIMESTAMP), MONTH) AND DATE_TRUNC(CAST('{{ end_datetime }}' AS TIMESTAMP), MONTH)
    AND COALESCE(tpep_pickup_datetime) IS NOT NULL
    AND COALESCE(tpep_dropoff_datetime) IS NOT NULL
  ...
  SELECT
    ct.pickup_time,
    ct.dropoff_time,
    TIMESTAMP_DIFF(ct.dropoff_time, ct.pickup_time, SECOND) AS trip_duration_seconds,
    ...
    ct.payment_type,
    pmt.payment_type_name AS payment_description,
    ct.extracted_at,
    CURRENT_TIMESTAMP AS updated_at

  FROM normalized_trips ct
  LEFT JOIN ingestion_hw5.taxi_zone_lookup AS pl
    ON ct.pickup_location_id = pl.locationid

  LEFT JOIN ingestion_hw5.taxi_zone_lookup AS dl
    ON ct.dropoff_location_id = dl.locationid
  
  LEFT JOIN ingestion_hw5.payment_lookup AS pmt
  ...
  WHERE 1=1
    ...
    AND TIMESTAMP_DIFF(ct.dropoff_time, ct.pickup_time, SECOND) > 0
    -- filter out outlier durations that are too long, 8 hours (28800 seconds)
    AND TIMESTAMP_DIFF(ct.dropoff_time, ct.pickup_time, SECOND) < 28800
