...
custom_checks:
  - name: positive_trip_count
    ...
    query: SELECT COUNT(*) FROM reports_hw5.trips_report WHERE total_trips <= 0
    value: 0
  - name: non_negative_revenue
    description: Ensures aggregated total_amount_total is non-negative
    query: SELECT COUNT(*) FROM reports_hw5.trips_report WHERE total_amount_total < 0
    value: 0
...
trips_by_month AS ( -- Step 1: Extract month from pickup_time and prepare data for aggregation, filtering for only charged trips
  SELECT
    taxi_type,
    DATE_TRUNC(pickup_time, MONTH) AS month_date,
  ...
  WHERE 1=1
    AND DATE_TRUNC(pickup_time, MONTH) BETWEEN DATE_TRUNC(CAST('{{ start_datetime }}' AS TIMESTAMP), MONTH) AND DATE_TRUNC(CAST('{{ end_datetime }}' AS TIMESTAMP), MONTH)
